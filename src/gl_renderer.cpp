...
bool gl_init(BumpAllocator* transientStorage){
    ...
    { // Test if vert shader compiled
      int succ;
      char shaderLog[2048] = {};

      glGetShaderiv(vertShaderID, GL_COMPILE_STATUS, &succ);
      if(!succ){
    ...
    { // Test if frag shader compiled
      int succ;
      char shaderLog[2048] = {};

      glGetShaderiv(fragShaderID, GL_COMPILE_STATUS, &succ);
      if(!succ){
...
void gl_render(){
  ...
  glClearColor(119.0f / 255.05f, 33.0f / 255.0f, 111.0f/255.0f, 1.0f);
  glClearDepth(0.0f);
